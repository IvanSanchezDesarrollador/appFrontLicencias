...
import { Text, Title, LineChart, BadgeDelta } from "@tremor/react";
import Loading from '../../../../components/Loading';


export default function GrafLineChartIV2({anio}) {
    const {
    ...
    const calcularTotalRecaudadoPorRangoFechas = (infractionsData, fechaInicio, fechaFin) => {
        ...
        const totalRecaudado = infraccionesEnRango.reduce(
            (acumulador, { multa_internamineto }) => acumulador + parseFloat(multa_internamineto ?? 0),
            0
        );
    ...
    return (
        <div className='overflow-x-scroll xl:overflow-auto'>
            <div className="2xl:flex justify-between items-center px-3">
                ...
                <Title className='text-violet-500'>Año: {anio}</Title>
            </div>
            <LineChart
                className="md:w-full w-[60rem] mb-4 2xl:mb-0 xl:mb-2"
                data={chartdata} 
                index="year"
